...
type UploadDraft = { id: string; file: File; preview: string; project: string; newProject: boolean; word: string; story: string; current: boolean };
...
export default function Workspace() {
  ...
  function addPhotos(files: File[]) {
    ...
    setDrafts((current) => [...current, ...files.map((file) => ({ id: crypto.randomUUID(), file, preview: URL.createObjectURL(file), project: "", newProject: false, word: file.name.replace(/\.[^.]+$/, ""), story: "", current: true }))]);
  }
  ...
  async function removeWork(item: Work) {
    if (!window.confirm(`Permanently delete “${item.word}”? This cannot be undone.`)) return;
    setBusy(true);
    setNotice("Deleting image and files...");
    const paths = [...new Set([item.display_image_path, item.raw_file_path].filter(Boolean))];
    const { error: storageError } = await supabase.storage.from("submissions").remove(paths);
    if (storageError) { setBusy(false); return setNotice("The image files could not be deleted. Please try again."); }
    const { error } = await supabase.from("submissions").delete().eq("id", item.id);
    setBusy(false);
    if (error) return setNotice("The gallery record could not be deleted. Please try again.");
    setNotice("Image permanently deleted.");
    await load();
  }

  const existingProjects = [...new Set(works.map((item) => item.project).filter(Boolean))].sort();
  ...
  return <main className="account-page">{header}<section className="account-intro"><p>IMAGE PUBLISHER</p><h1>YOUR<br />WORK.</h1><div className="account-email">{user.email}</div><form className="submission-form" onSubmit={publish}><span>ADD TO PORTFOLIO</span><label className="upload-picker">CHOOSE PHOTOS<input type="file" accept="image/*" multiple onChange={(e) => { const selected = Array.from(e.currentTarget.files ?? []); e.currentTarget.value = ""; addPhotos(selected); }} /><small>Choose as many images as you want. Their details stay editable below.</small></label>{drafts.map((draft, index) => <article className="upload-draft" key={draft.id}><div className="upload-preview"><img src={draft.preview} alt="" /></div><div className="upload-draft-head"><strong>{String(index + 1).padStart(2, "0")}</strong><span>{draft.file.name}</span><button type="button" onClick={() => setDrafts((current) => current.filter((item) => item.id !== draft.id))}>REMOVE</button></div><label>PROJECT<input list="existing-projects" value={draft.project} onChange={(e) => updateDraft(draft.id, { project: e.target.value })} required placeholder={existingProjects.length ? "Choose a saved project or type a new one" : "Type a new project name"} /></label><label>IMAGE TITLE<input value={draft.word} onChange={(e) => updateDraft(draft.id, { word: e.target.value })} required placeholder="e.g. NIGHT WALK" /></label><label>NOTES<textarea value={draft.story} onChange={(e) => updateDraft(draft.id, { story: e.target.value })} placeholder="Optional context" /></label><label className="current-toggle"><input type="checkbox" checked={draft.current} onChange={(e) => updateDraft(draft.id, { current: e.target.checked })} />SHOW ON HOMEPAGE <small>Unchecked images remain in the gallery, outside the rotating selection.</small></label></article>)}<datalist id="existing-projects">{existingProjects.map((project) => <option key={project} value={project} />)}</datalist><button disabled={busy || !drafts.length}>{busy ? "UPLOADING..." : `PUBLISH ${drafts.length || ""} IMAGE${drafts.length === 1 ? "" : "S"} →`}</button></form><small className="account-notice">{uploadProgress || notice || "No submission limit. Publish whenever you are ready."}</small><div className="my-work"><span>PORTFOLIO INDEX</span>{works.map((item) => <article key={item.id}><div><strong>{item.word}</strong><small>{item.project} / {item.status.toUpperCase()}</small></div><div className="work-actions"><button onClick={() => void archive(item)}>{item.status === "archived" ? "SHOW IN ROTATION" : "REMOVE FROM ROTATION"}</button><button className="delete-work" disabled={busy} onClick={() => void removeWork(item)}>DELETE IMAGE</button></div></article>)}</div><button className="signout" onClick={() => void supabase.auth.signOut()}>SIGN OUT</button></section></main>;
}

...
export default function Home() {
  ...
  return <main className="gallery-shell portfolio-shell">
    ...
    <section className="stage" id="top">
      ...
      {works.map((item, index) => <button className={`frame ${index === active ? "is-active" : ""}`} key={item.id} onClick={() => { setActive(index); setOpen(true); }} aria-label={`Open ${item.word}`}><img src={item.image} alt="" /></button>)}
      <button className="work-caption" onClick={() => setOpen(true)}><span className="work-word" data-text={work.word}>{work.word}</span><span className="work-meta">{work.project.toUpperCase()} / 2026</span></button>
    ...
    {open && <div className="overlay" role="dialog" aria-modal="true" onClick={() => setOpen(false)}><article onClick={(event) => event.stopPropagation()}><button className="close" onClick={() => setOpen(false)} aria-label="Close work">×</button><img src={work.image} alt={work.word} /><div className="detail"><p className="detail-word">{work.word}</p><p>{work.story}</p><span>{work.project.toUpperCase()} / 2026</span></div></article></div>}
    {archiveOpen && <div className="overlay archive-overlay" role="dialog" aria-modal="true" onClick={() => setArchiveOpen(false)}><article className="archive-card" onClick={(event) => event.stopPropagation()}><button className="close" onClick={() => setArchiveOpen(false)} aria-label="Close archive">×</button><div className="archive-intro"><span>INSIGHTFOOLISH / PROJECT INDEX</span><h1>ARCHIVE</h1><p>Images are held here by the project they belong to.</p></div>{Object.entries(archive).map(([project, entries]) => <section className="project-group" key={project}><h2>{project}</h2><div className="archive-grid">{entries.map((item) => <button key={item.id} onClick={() => { setActive(works.findIndex((entry) => entry.id === item.id)); setArchiveOpen(false); setOpen(true); }}><img src={item.image} alt="" /><span>{item.word}</span><small>{project}</small></button>)}</div></section>)}</article></div>}
